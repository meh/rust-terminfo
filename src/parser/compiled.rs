...
mod test {
    ...
    fn standard() {
        load("tests/st-256color", |db| {
            ...
            assert_eq!(
                Some(cap::AutoRightMargin(true)),
                db.get::<cap::AutoRightMargin>()
            );
            assert_eq!(
                Some(cap::AutoLeftMargin(false)),
                db.get::<cap::AutoLeftMargin>()
            );
        });
    }
    ...
    fn extended() {
        load("tests/cancer-256color", |db| {
            ...
            assert_eq!(
                Some(&cap::Value::String(b"\x1B[2 q".to_vec())),
                db.raw("Se")
            );
        });
    }
