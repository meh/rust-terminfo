...
named!(percent<Item>, value!(Item::String(b"%"), tag!("%")));

named!(constant<Item>, alt!(constant_char | constant_integer));

named!(
    constant_char<Item>,
    do_parse!(
        tag!("'") >> ch: take!(1) >> tag!("'") >> (Item::Constant(Constant::Character(ch[0])))
    )
);

named!(
    constant_integer<Item>,
    do_parse!(
        tag!("{")
...
named!(
    ...
    do_parse!(
        opt!(tag!(":"))
            ...
            >> precision:
                opt!(do_parse!(
                    tag!(".") >> amount: take_while!(is_digit) >> (amount)
                ))
